...
class FileParser {
    ...
    extractCode(AST) {
        ...
        let matches = esquery.match(AST, esquery.parse(`[expression.callee.object.name = 'Ext'][expression.callee.property.name = define]`));
        if (matches && matches[0] && matches[0].expression.arguments) {
            me.addName(matches[0].expression.arguments[0].value);
        }

        matches = esquery.match(AST, esquery.parse(`ObjectExpression > Property[key.name=alternateClassName]`));
        ...
        if (!this.ignoreOverrides) {
            matches = esquery.match(AST, esquery.parse(`ObjectExpression > Property[key.name=override]`));
            if (matches && matches[0] && matches[0].value) {
                let node = matches[0].value;
                if (node.type === "Literal") {
    ...
    parseTag(tag) {
        ...
        if (tag.type === "class" && (tag.name === "Ext" || tag.name === "Ext.Widget")) {
        ...
        if (overrideTags.includes(tag.type)) {
